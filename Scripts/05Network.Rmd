...
# Load functions
source("./Scripts/Functions/AssociationFrequencies.R")
```
```{r, echo=FALSE}
# Create new directory for Network data
if(!file.exists("Data/NetworkData")){dir.create("Data/NetworkData")}
if(!file.exists("Results/Networks")){dir.create("Results/Networks")}
```

## 01 Prevalence filter

It has been shown that co-absence can yield high correlation values. Hence, we only considered genera present in all samples are used for network calculation. Further we filtered for genera accounting for more than 0.01 % across all samples. All genera that did not meet the critera were combined into a pseudo-taxon. 
...
For network visualization, we summarized the number of associations and the ratio of positive to negative associations for primary producers, decomposers and consumers as well as for microbial groups, i.e algae and cyanobacteria, fungi, other bacteria, predatory bacteria, protists and microscopic metazoans.
...
Additionally, we show the summarized number of associations number of associations (point size) and the ratio of positive to negative associations (point color)  also as point graph.
...
# Save graph as plot
ggsave(paste0("Results/Networks/Edges_BetweenTrophicLevels.pdf"), 
       plot = g1, width = 3.5, height = 4.9)
g=g1+theme(axis.text.y = element_blank())
ggsave(paste0("Results/Networks/Edges_BetweenTrophicLevels_NL.pdf"), 
       plot = g, width = 3.5, height = 4.9)

# Save graph as plot
ggsave(paste0("Results/Networks/Edges_WithinTrophicLevels.pdf"), 
       plot = g2, width = 3.5, height = 4.2)
g=g2+theme(axis.text.y = element_blank())
ggsave(paste0("Results/Networks/Edges_WithinTrophicLevels_NL.pdf"), 
       plot = g, width = 3.5, height = 4.2)
```
```{r, echo=FALSE, results='hide'}
# Clear console and workspace
cat("\014") 
...
Lastly, we create pie charts for each microbial group to show the community composition.  
